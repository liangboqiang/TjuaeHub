...
for (const reg of registries) {
  ...
  console.log(`bun install --cwd ${dir} ${PKG} --registry=${reg.url}`);
  try {
...
const link = join(binDir, 'auggie');
if (!existsSync(link)) {
  symlinkSync(join(dir, 'node_modules', '.bin', 'auggie'), link);
}
