...
// Priority: 1) TJUAE_FONTS_DIR env var, 2) skills/canvas-design relative path, 3) system fonts
function getFontDir() {
  const candidates = [
    process.env.TJUAE_FONTS_DIR,
    path.join(__dirname, '../../canvas-design/canvas-fonts'),
    path.join(process.env.HOME || '', 'Library/Application Support/TjuaeUI/config/skills/canvas-design/canvas-fonts'),
    path.join(process.env.APPDATA || '', 'TjuaeUI/config/skills/canvas-design/canvas-fonts'),
  ].filter(Boolean);
