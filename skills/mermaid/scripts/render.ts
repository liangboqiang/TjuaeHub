 ...
 * Usage:
 *   npx --no-install tsx render.ts <mermaid-file> [--ascii] [--theme <theme>] [--output <file>]
 *   echo "graph TD; A-->B" | npx --no-install tsx render.ts --stdin [--ascii]
 */
...
async function loadMermaid() {
  try {
    ...
    throw new Error('缺少锁定依赖，请先在技能目录运行 npm ci --ignore-scripts');
  }
}
...
Usage:
  npx --no-install tsx render.ts <file.mmd> [options]
  echo "graph TD; A-->B" | npx --no-install tsx render.ts --stdin [options]
...
Examples:
  npx --no-install tsx render.ts diagram.mmd --output diagram.svg
  npx --no-install tsx render.ts diagram.mmd --ascii
  echo "graph LR; A-->B-->C" | npx --no-install tsx render.ts --stdin --ascii
`);
    process.exit(0);
  }
